...
$args = array(
  'post_type' => 'page',
  'posts_per_page' => -1, // Para obter todas as páginas
  'post_status' => 'publish',
  'post_parent' => 0,
  'orderby' => 'ID',
  'order' => 'ASC',
...
// Passo 2: Iterar sobre os resultados
if ($query->have_posts()) {
      ...
      switch ($page_id) {
        case '7':
          $home[] = array(
            ...
            'url' => $page_url,
            'children' => $children,
            'class' =>  $page_id == $id ? 'active' : '',
            'aria-current' => $page_id == $id ? 'aria-current = "page" ' : ''
          );
          break;
        case '67':
          $contato[] = array(
...
/*
echo '<pre>';
print_r($plataformas[0]['children']);
echo '</pre>';
*/
?>
  ...
  <div class="container">
    <div class="row">
      <div class="col-12 d-flex align-items-baseline justify-content-between">
        <div class="logo">
            <a aria-label="link para voltar para a home" class="navbar-brand" href="<?= get_blogInfo('url') ?>"><img width="150px" src="<?= get_template_directory_uri() . '/assets/img/logo_white_icon2.svg'  ?>" alt="Logo Setos Plataformas Elevatórias"></a> 
        </div><!-- end logo -->
        ...
        </div> <!-- end container nav + whatsapp -->
      </div><!-- end col -->
    </div><!-- end row -->
  </div><!-- end container -->
</header>
